...
KANATA_CONFIG_REL_PATH=".config/kanata/kanata-config.kbd"
...
echo "KERNEL==\"uinput\", MODE=\"0660\", GROUP=\"$UINPUT_GROUP\", OPTIONS+=\"static_node=uinput\"" >/etc/udev/rules.d/99-input.rules
...
User=$KANATA_USER
Group=$KANATA_USER
ExecStart=/usr/bin/sh -c 'exec /usr/bin/kanata --quiet --cfg $SYSTEM_KANATA_CONFIG_DST'
Restart=no
# Security
